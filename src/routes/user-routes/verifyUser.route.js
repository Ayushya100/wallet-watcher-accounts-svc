...
const verifyUser = async(req, res, next) => {
    ...
    try {
        ...
        const isValidPayload = userManagementSetting.validateUserVerificationPayload(verificationCode);

        if (isValidPayload.isValid) {
            log.info('Call controller function to check if user exists');
            const isUserExist = await userManagementSetting.checkUserById(userId);

            if (isUserExist.isValid) {
                log.info('Call controller function to verify user');
                const isUserValidated = await userManagementSetting.verifyUser({userId, verificationCode});
                
                if (isUserValidated.isValid) {
                    registerLog.createInfoLog('New user registered successfully', isUserValidated);
                    
                    const mailPayload = userManagementSetting.sendVerificationSuccessfulMailPayload(isUserValidated.data);
